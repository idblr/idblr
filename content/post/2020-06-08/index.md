...
I present an update to my previous posts [#1](/post/covid-dc) and [#2](/post/covid-dc-test). This update can also be found on a public [GitHub repository](https://github.com/idblr/coviDC). 

Starting May 17, 2020 the DC Mayoral Office began releasing testing information by neighborhood on their [coronavirus data portal](https://coronavirus.dc.gov/page/coronavirus-data). Molly Tolzmann [zmotoly](https://twitter.com/zmotoly) added this feature to her publicly available [Google Sheet](https://docs.google.com/spreadsheets/d/1u-FlJe2B1rYV0obEosHBks9utkU30-C2TSkHka6AVS8/edit#gid=1923705378) presented at the [DC health planning neighborhood level](https://opendata.dc.gov/datasets/dc-health-planning-neighborhoods). 

Here, I present the daily 5-day rolling average test positivity rate (TPR) for COVID-19 cases from May 25 to June 17, 2020 in an animated Graphics Interchange Format (GIF) using the open-source software language [R](https://cran.r-project.org/). A TPR is the proportion of positive tests for every administered test. For example, the World Health Organization [recommends](https://coronavirus.jhu.edu/testing/testing-positivity) a test positivity rate below 5% (1 out of 20 tests, TPR = 0.05) before reopening. A 5-day rolling average is the mean value of a daily TPR and the TPR of the four days prior. 

**Important Note**: There are missing data for certain dates. Case information is missing for May 22nd and 27th. Testing information is missing for May 22nd, 23rd, 24th, and 27th. Missing data are considered `NA` but included in the 5-day rolling averages.  Therefore, 5-day rolling averages that include these dates are more unstable. Because reporting case information at the [DC health planning neighborhood level](https://opendata.dc.gov/datasets/dc-health-planning-neighborhoods) became available on May 7th, daily incident case information is available starting May 8th and 5-day rolling average cases are available starting May 12th. Because testing information became available on May 20th, daily testing information is available starting May 21st and 5-day averages are available starting May 25th.
...
I merge the District of Columbia [Health Planning Neighborhoods](https://opendata.dc.gov/datasets/acs-2018-population-variables-tract/data) boundaries and Molly Tolzmann's [zmotoly](https://twitter.com/zmotoly) [collation](https://docs.google.com/spreadsheets/d/1u-FlJe2B1rYV0obEosHBks9utkU30-C2TSkHka6AVS8/edit#gid=1923705378) of the cumulative cases and tests from start of the SARS-CoV-2 outbreak. I clean up variable names that are easier for [R](https://cran.r-project.org/) to use. 
...
The 5-day rolling average test positivity rate is not provided in Molly Tolzmann's [zmotoly](https://twitter.com/zmotoly) [Google Sheet](https://docs.google.com/spreadsheets/d/1u-FlJe2B1rYV0obEosHBks9utkU30-C2TSkHka6AVS8/edit#gid=1923705378) and I calculate it for every day data are available. 
...
I use the [ggplot2](https://github.com/tidyverse/ggplot2) and [gganimate](https://github.com/thomasp85/gganimate) packages to create a GIF of the daily 5-day rolling average TPR for COVID-19 cases from May 25 to June 17, 2020. The packages require converting the data from wide- to long-format. 
...
The following is the code to create a GIF of the daily 5-day rolling average TPR for COVID-19 cases from May 25 to June 17, 2020. I chose [non-alarmist colors](https://blog.mapbox.com/7-best-practices-for-mapping-a-pandemic-9f203576a132?gi=6907699c528e) similar to previous posts [#1](/post/covid-dc) and [#2](/post/covid-dc-test). Grey-colored areas have missing (`NA`) or no information. 
...
<img src="{{< blogdown/postref >}}index_files/figure-html/animation-1.gif" alt="" style="display: block; margin: auto;" />

Overall, my qualitative assessment of the daily 5-day rolling average TPR for COVID-19 cases from May 25 to June 17, 2020 is a decrease in TPR overtime, especially after June 3rd. Ward 4 (Northern-most section) appears to have consistently high 5-day rolling average TPR overtime. 

**The provided maps are not intended to inform decision-making**. Instead, I provide the the [open-source code](https://github.com/idblr/coviDC) to download, manage, and visualize publicly available data. Future steps (in addition to one listed in my [previous post](/post/covid-dc)) include, displaying TPR above or below the the 5\% WHO recommendation and conducting spatial statistical analysis such as, for example, assessing the presence of spatio-temporal clustering. Other values (e.g., daily TPR, incident cases per 1,000) can be animated by modifying the above code, which is also provided in my public [GitHub repository](https://github.com/idblr/coviDC).

Thanks, again, to Molly Tolzmann [zmotoly](https://twitter.com/zmotoly) for the data collation, management, and custodianship.  
